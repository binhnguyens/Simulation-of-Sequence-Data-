...
def genfrags(seq, minlen, maxlen, requiredCoverage) :
    ...
    while not covered() :
        ...
        if coverage [0] <=requiredCoverage-1:
            start = 0
            end = start + randlen
           
            

        elif coverage [0]>requiredCoverage-1 and coverage [n-1]<=requiredCoverage-1:
            end = n
            start = end - randlen    
    ...
    # -----------------------------------------------------
    print ('This is the coverage matrix')
    print coverage
    
    print ('The average is', np.mean(coverage))
